...
export default async function TheMovieDatabaseRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: FastifyQuery; Params: FastifyParams }>, reply: FastifyReply) => {
      ...
      if (category !== 'popular' && category !== 'top') {
        return reply.status(400).send({ error: `Invalid category: '${category}'. Expected 'popular' , 'top'` });
      }
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: FastifyQuery; Params: FastifyParams }>, reply: FastifyReply) => {
      ...
      if (category !== 'popular' && category !== 'top' && category !== 'seasonal' && category !== 'weekly') {
        return reply
          .status(400)
          .send({ error: `Invalid category: '${category}'. Expected 'popular' , 'top' ,'weekly' or 'seasonal'` });
      }
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: FastifyQuery; Params: FastifyParams }>, reply: FastifyReply) => {
      ...
      const cacheKey = `tmdb-${category}-artwoks`;
      const cachedData = await redisGetCache(cacheKey);
      if (cachedData) return reply.status(200).send(cachedData);
