...
export default async function AnimeHeavenRoutes(fastify: FastifyInstance) {
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: FastifyQuery }>, reply: FastifyReply) => {
      ...
      const cacheKey = `animeheaven-search-${q}`;
      const cachedData = await redisGetCache(cacheKey);
      if (cachedData) return reply.status(200).send(cachedData);
      try {
        const result = await animeheaven.search(q);
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Params: FastifyParams }>, reply: FastifyReply) => {
      ...
      let duration;
      const cacheKey = `animeheaven-info-${id}`;
      const cachedData = await redisGetCache(cacheKey);
      if (cachedData) {
        return reply.status(200).send(cachedData);
      }
      try {
        const result = await animeheaven.fetchAnimeInfo(id);
        if (!result || typeof result !== 'object') {
          return reply.status(502).send({
            error: 'External provider returned an invalid response(null)',
          });
  ...
  fastify.get(
    ...
    async (request: FastifyRequest<{ Querystring: FastifyQuery; Params: FastifyParams }>, reply: FastifyReply) => {
      ...
      const cacheKey = `animeheaven-sources-${episodeId}`;
      const cachedData = await redisGetCache(cacheKey);
      if (cachedData) {
        return reply.status(200).send(cachedData);
      }
      try {
        const result = await animeheaven.fetchSources(episodeId);
        if (!result || typeof result !== 'object') {
          return reply.status(502).send({
            error: 'External provider returned an invalid response(null)',
          });
